...
// Project-local settings are ignored unless Pi marks the project trusted.

import { readFileSync, existsSync } from "node:fs";
import { join } from "node:path";
import { DEFAULT_CONFIG, type HandoffConfig } from "./types.ts";
 ...
 * @param projectTrusted - Whether Pi has approved project-local configuration
 * @returns Merged configuration
 */
export function loadConfig(cwd: string, projectTrusted: boolean): HandoffConfig {
  if (!projectTrusted) return mergeConfig(undefined);
  const overrides = readSettingsFile(cwd);
  return mergeConfig(overrides);
}

export function validateGoal(goal: string, minLength: number): GoalValidation {
