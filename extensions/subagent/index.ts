...
const SubagentParams = Type.Object({
	...
	cwd: Type.Optional(Type.String({ description: "Working directory inside the parent workspace (single mode)" })),
});
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			`Project-local agents in ${CONFIG_DIR_NAME}/agents require trusted-project state and explicit interactive approval.`,
			"Custom child working directories must remain inside the parent workspace.",
		].join(" "),
		parameters: SubagentParams,

		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			const agentScope: AgentScope = params.agentScope ?? "user";
			const includesProjectAgents = agentScope === "project" || agentScope === "both";
			assertProjectAgentAccess(includesProjectAgents, ctx.isProjectTrusted(), ctx.hasUI);
