...
// This fork neutralizes terminal controls in user and model display text.
...
import { getRequestAuth } from "../shared/auth.ts";
import {
  BTW_SYSTEM_PROMPT,
  buildBtwUserMessage,
  validateBtwArgs,
...
class BtwOverlay implements Component {
  ...
  constructor(
    ...
    this.tui = tui;
    this.theme = theme;
    this.question = prepareBtwDisplayText(question);
    this.answer = prepareBtwDisplayText(answer);
    this.onDone = onDone;
  }
...
async function runBtwCommand(
  ...
  if (!ctx.hasUI) {
    ...
    if (response.stopReason === "error") {
      console.error(prepareBtwDisplayText(response.errorMessage ?? "LLM error"));
      return;
    }

    const answerText = prepareBtwDisplayText(extractResponseText(response.content));
    console.log(`\n> btw: ${prepareBtwDisplayText(question)}\n`);
  ...
  const answerResult = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
    const loader = new BorderedLoader(
      tui,
      theme,
      `Thinking (${prepareBtwDisplayText(btwModel.id)})...`,
    );
    ...
    doQuery()
      ...
      .catch((err) => {
        ...
        console.error(`BTW query failed: ${prepareBtwDisplayText(message)}`);
        done(null);
      });
  ...
  const answer = normalizeBtwAnswer(answerResult);
  if (answer === null) {
  ...
  await ctx.ui.custom<void>((tui, theme, _kb, done) => {
    return new BtwOverlay(tui, theme, question, answer, done);
  }, {
    overlay: true,
    overlayOptions: {
      anchor: "center",
